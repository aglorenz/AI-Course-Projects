    ...
    for iteration in range(num_iterations):
        ...
        for ant in range(num_ants):
            ...
            for _ in range(num_cities - 1):
                ...
                selection_probs = (
                    pheromone_level[current_city] ** alpha) * (heuristic_info[current_city] ** beta)
                # Set selection probabilities of visited cities to 0
                selection_probs[np.array(visited)] = 0

                # Choose the next city based on the selection probabilities
                next_city = np.random.choice(np.arange(num_cities), p=(
                    selection_probs / np.sum(selection_probs)))

                # Update the path and visited ist
            ...
            ant_distances[ant] += distance_matrix[current_city,
                                                  ant_paths[ant, 0]]
        ...
        for ant in range(num_ants):
            for city in range(num_cities - 1):
                pheromone_level[ant_paths[ant, city],
                                ant_paths[ant, city+1]] += 1 / ant_distances[ant]
            pheromone_level[ant_paths[ant, -1],
                            ant_paths[ant, 0]] += 1 / ant_distances[ant]
...
best_path, best_distance, best_iteration = ant_colony_optimization(
    num_iterations)
